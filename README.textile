...
# ensure you've got rubygems installed, e.g. by running (as admin/root) @apt-get install rubygems@
# ensure you've got rake installed, e.g. by running @apt-get install rake@ or @gem install rake@
# make sure you've got @qmake@ and the QT4 libraries (and developemnt headers!) on your system (those are required by capybara-webkit, which is a dependency itself)
# @gem install bundler@
# @bundle install@
